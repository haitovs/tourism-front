...
async def get_latest_terms(req: Request, *, site_id: Optional[int] = None) -> Optional[dict]:
    import logging

    log = logging.getLogger("services.terms")
    try:
        data = await api_get(req, "/terms-of-use/latest", soft=True)
        if not data:
            return None
        ...
        return {
            "id": data.get("id"),
            "title": data.get("title") or "Terms of Use",
        ...
        log.exception("get_latest_terms unexpected: %r", exc)
        return None
